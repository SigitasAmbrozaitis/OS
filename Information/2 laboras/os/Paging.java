...
public class Paging {
    ...
    public void findFreeBlock()
    {
        int count = 0;
        this.blockNumber = -1;
        for(int i = 0; i < (OS.RM_MEMORY_SIZE / 10); i++)
        {
            for(int j = 0; j < 10; j++)
            {
    ...
    public void findPTR()
    {
        ...
        if(blocksFound == blocksToFind)
        {
            for(int i = 0; i < blocksToFind; i++)
            {
                OS.rmMemory[ptrBlock + i].setCell(String.valueOf(blocksFinded[i]));
                OS.rmMemory[ptrBlock + i].setState(true);
            }
        }
        else
        {
        ...
        OS.realMachine.setRegisterPTR(((blocksToFind - 1) * 10) + ptrBlock);
    }
    ...
    public void addMore()
    {
        ...
        else
        {
            ...
            if (blocksFound == blocksToFind)
            {
                ...
                OS.realMachine.setRegisterPTR((((OS.realMachine.getRegisterPTR() / 100) + blocksToFind) * 10) + ptr);
            }
            else
            {
                OS.realMachine.setRegisterPI(3);
